...
class APRSBBS:
    ...
    def __init__(
        ...
        # ------------------------------------------------------------------
        # A simple SQLite database is used to persist private messages on disk.
        # This allows the BBS to recover queued mail after a restart or crash.
        # The database will be created automatically in the same directory
        # as this script if it does not already exist.  We wrap all
        # database operations in a lock to avoid concurrency issues as
        # the receiver thread and main thread may both access the database.
        self._db_lock = threading.Lock()
        # Determine path for the SQLite database.  Use the APRS_BBS_DB_PATH
        # environment variable if set, otherwise default to 'aprs_bbs.db'
        # in the current working directory.  This allows advanced users
        # to override the location without modifying the script.
        env_db_path = os.getenv("APRS_BBS_DB_PATH")
        if env_db_path:
            self.db_path = env_db_path
        else:
            # Place the database alongside the script file
            self.db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "aprs_bbs.db")
        # Initialises the SQLite database and loads any undelivered
        # messages into in-memory mailboxes.
        self._init_db()
    ...
    def _store_private_message(self, from_call: str, to_call: str, message: str) -> None:
        ...
        # Add the message to the in-memory mailbox
        self.mailboxes.setdefault(to_call, []).append((from_call, message))
        # Persist the message in the SQLite database.  Wrap in a
        # try/except so that a database error doesn't disrupt normal
        # operation; if the write fails the message will still be
        # delivered for the current session, but may not survive a
        # restart.
        try:
            self._save_message_to_db(from_call, to_call, message)
        except Exception as exc:
            self.logger.error("Failed to write message to database: %s", exc)
    ...
    def _handle_login(self, callsign: str) -> None:
        ...
        if inbox:
            for from_call, message in inbox:
                ...
                # Mark the message as delivered in the database.  Use a
                # broad update to catch all matching rows; duplicates
                # (same sender and message) will all be flagged as
                # delivered.  If the update fails, it is logged but
                # doesn't interrupt message delivery.
                try:
                    self._mark_message_delivered(from_call, callsign, message)
                except Exception as exc:
                    self.logger.error(
                        "Failed to mark message as delivered for %s->%s: %s", from_call, callsign, exc
                    )
            # Clear delivered messages from in-memory mailbox
    ...
    def _init_db(self) -> None:
        ...
        This method sets up a single table called 'messages' to store
        private messages for later delivery.  Each row has an auto
        incrementing primary key, the sender and recipient callsigns,
        the message body, a delivered flag (0 for pending, 1 once
        delivered) and a UNIX timestamp of when the message was stored.
        """
        with self._db_lock:
    ...
    def _save_message_to_db(self, from_call: str, to_call: str, message: str) -> None:
        """Insert a message into the SQLite database.

        Messages are inserted with delivered=0 so they will be sent
        the next time the recipient logs in.  SQLite handles concurrent
        access via internal locking; we additionally protect access
        ...
        with self._db_lock:
            ...
            try:
                conn.execute(
                    "INSERT INTO messages (from_call, to_call, message, delivered) VALUES (?,?,?,0)",
                    (from_call.upper(), to_call.upper(), message),
                )
                conn.commit()
            finally:
                conn.close()

    def _load_undelivered_messages(self) -> None:
        """Load undelivered messages from the database into mailboxes.

        On start-up the BBS inspects the messages table and pulls
        any rows where delivered=0.  These are added to the in-memory
        mailboxes so that they will be delivered when users log in.
        """
        with self._db_lock:
            conn = sqlite3.connect(self.db_path)
            try:
                cur = conn.execute(
                    "SELECT from_call, to_call, message FROM messages WHERE delivered=0"
                )
                rows = cur.fetchall()
                for from_call, to_call, message in rows:
                    self.mailboxes.setdefault(to_call.upper(), []).append((from_call.upper(), message))
            finally:
                conn.close()

    def _mark_message_delivered(self, from_call: str, to_call: str, message: str) -> None:
        """Set delivered=1 for matching messages in the database.

        This method updates all rows that match the given sender,
        recipient and message text and have not yet been delivered.
        It is called after the message has been sent to the client.
        """
        with self._db_lock:
            conn = sqlite3.connect(self.db_path)
            try:
                conn.execute(
                    "UPDATE messages SET delivered=1 WHERE from_call=? AND to_call=? AND message=? AND delivered=0",
                    (from_call.upper(), to_call.upper(), message),
                )
                conn.commit()
            finally:
